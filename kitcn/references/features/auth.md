...
```ts
import { boolean, convexTable, defineSchema, id, index, integer, text, timestamp } from 'kitcn/orm';
...
# Auto-generated by: npx kitcn env sync --auth
BETTER_AUTH_SECRET=...
JWKS=...
```

`npx kitcn env sync --auth` behavior:
1. Syncs `convex/.env` values into the active deployment.
2. Auto-generates `BETTER_AUTH_SECRET` + `JWKS` when missing.

Prerequisite:
1. Run `npx kitcn dev` first so deployment connection is active.
