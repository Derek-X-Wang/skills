...
1. `kitcn dev` delegates to `convex dev` and may prompt for interactive setup when bootstrap is missing.
...
4. Then run `bunx kitcn dev` (this already runs codegen/API generation).
...
1. Default to `bunx kitcn dev` (or one-shot `bunx kitcn dev --once --typecheck disable`).
2. `kitcn dev` already runs codegen/API generation.
3. Do not run `bunx kitcn codegen` as a separate default step.
4. Use manual `bunx kitcn codegen` only as fallback when `kitcn dev` cannot be run and backend is already active.
...
1. `kitcn codegen` still requires an active Convex backend connection.
2. If you see `Local backend isn't running`, start `bunx kitcn dev` in another terminal and retry.
3. If this remains blocked in agent mode, pause and ask the user to run `bunx kitcn dev`, then continue after it is live.
...
If Aggregates are **disabled**, remove `aggregateIndex`/`rankIndex` declarations from table definitions and re-run `bunx kitcn dev --once --typecheck disable`.
...
Use `Ratelimit` from `kitcn/plugins/ratelimit`:

```ts
import { MINUTE, Ratelimit } from "kitcn/plugins/ratelimit";
