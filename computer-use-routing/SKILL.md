...
Before the first UI action, identify only what can change the adapter decision: the executing session and machine, adapter and target state owner, and any required host/state affinity. In worker mode, reuse the orchestrator's assigned harness/model/effort/route and supplied runtime context; do not repeat host or quota investigation. Check actual task tools/access and the assigned target's adapter eligibility locally, and report mismatches to the orchestrator. A supplied identity never proves inherited desktop/plugin readiness. Then record:
...
Trust injected runtime instructions and live tool schemas first. Discover connection and readiness through advertised read-only surfaces or visible state. Never inspect credential stores, cookies, browser profile files, or other secret-bearing storage to infer availability. When host identity changes adapter eligibility, use current session metadata, live exposed tools, and scoped read-only ownership checks; never infer it from the model, installed apps, a reachable Orca CLI, a shared catalog, or project directory. Leave unknown or conflicting fields unknown and resolve them only when they change a safe action, through the orchestrator in worker mode. Incomplete host identity alone is not a refusal when the adapter and target state owner are independently proven. Exposed T3 preview tools prove that browser surface only. Refresh affected adapter/target evidence after a handoff, reconnect, restart, or actual evidence change; the orchestrator owns renewed session routing.
...
origin: <originating host, harness, model, worker, session, machine, and task identifiers from the verified return path>
executors_attempted:
...
- The selected executor must verify its own task tools/access, assigned adapter eligibility, and actual target before acting, without repeating the orchestrator's model/effort/quota or host investigation. Treat prior actions and state as claims to verify, preserve the replay and disclosure constraints, and never repeat a non-idempotent mutation merely because the prior executor did not finish. If landing cannot be verified, do not re-attempt; return `PARTIAL` with the uncertainty. A claimed tool or open browser in another session is not proof of current eligibility.
- Cross-host dispatch ownership stays with `agent-orchestration`: it verifies the actual session route, delivery, approvals, return, and lifecycle. An open or prefilled composer or a shared folder is not proof of dispatch. An automatic route without that verification is unverified (`NOT-RUN`); propose a manual user-send handoff only through proper authority. Keep `PARTIAL` and its replay constraints when a non-idempotent action may have landed.
