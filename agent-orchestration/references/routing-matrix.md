...
| OpenCode Go | OpenCode | Curated multi-provider model pool centered on open-source models | OpenCode worker through an Orca-managed separate terminal; direct use from another harness is untested and not an approved route |
...
| OpenCode implementation worker | GLM-5.3-Flash in OpenCode through Orca | DeepSeek V4 Flash in OpenCode through Orca | Use GLM for stronger work. Use DeepSeek for much cheaper bounded implementation. Keep the current session as orchestrator. |
| Review of Claude work | GPT-5.6 Sol through a verified Codex route | A different reviewer-eligible Claude model, or GLM-5.3-Flash through Orca when provider diversity justifies the setup | Prefer model and provider diversity. Follow model-profile eligibility. |
| Review of GPT work | Opus through a verified Claude route | Fable, then GLM-5.3-Flash through Orca, then a fresh independent GPT session for R1 only | Use the first reviewer-eligible different-model route. A same-model GPT fallback is R1, or `PARTIAL` when R2 was required. |
...
| Either harness; GLM-5.3-Flash review | Orca-managed separate OpenCode terminal | Use when model or provider diversity justifies the setup cost. Verify the exact model, worktree, independence, and return path. |
...
Prefer abundant Codex usage for implementation volume and preserve scarce Fable capacity for difficult orchestration. Use OpenCode Go for cost-efficient external worker volume when the Orca setup cost is justified. Prefer high effort for frontier models as a starting point. Spend more only when representative work shows a useful quality gain. Use the smallest capable model for passive watchers and bounded observation roles.
