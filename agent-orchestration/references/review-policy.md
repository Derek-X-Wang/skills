...
When Astra is selected for important review or judging, default to high effort. Reserve targeted xhigh escalation for an unresolved critical correctness question or conflicting evidence; do not automatically repeat the whole review at xhigh. Honor an explicit effort request. This is Derek's operating preference dated 2026-09-09, not measured quality equivalence or subscription savings.

Fresh Astra can add an important same-model review of Astra work when it provides distinct value, but NEVER satisfies its cross-model R2 gate by itself, regardless of effort. It can satisfy cross-model review when the actual implementer is another model. This additional review is optional, not a two-reviewer requirement for every ordinary task; human and mixed authorship rules above remain intact.
