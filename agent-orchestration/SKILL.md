...
Select each worker's model explicitly for its task. Never inherit the orchestrator's model, including Fable or Astra, merely because the parent session uses it. Prefer a sufficient implementer for decided work under the dated role defaults in [routing-matrix.md](references/routing-matrix.md): GLM-5.3-Flash remains the global ordinary bounded implementation preference; GPT-5.6 Sol (`gpt-5.6-sol`) at high effort is the default for normal Codex implementation and research workers. Astra is specialist-only: important review or judging, explicitly invoked bounded repo audits, stalled-worker takeover, or a specifically identified difficult implementation scope whose concrete unresolved problem warrants Astra. Medium remains available for that bounded implementation exception or an explicit request, not routine execution or an entire ticket merely labeled complex. Reserve Fable for difficult judgment and explicitly authorized orchestrator handoffs; [review-policy.md](references/review-policy.md) owns its review escalation criteria and the Astra-authored R2 gate. Escalate to a stronger model only for a concrete unresolved difficulty, a failed bounded attempt, high uncertainty or impact that needs judgment, or a required tool. Never stage a deliberate weak-model failure first. State a concise selection rationale, not a verbose audit ritual.
...
Default new Codex orchestration to GPT-5.6 Sol at high effort; use Opus for normal Claude Code orchestration. Astra and Fable are not automatic new-orchestration defaults or quota fallbacks. When orchestration becomes genuinely difficult, the active orchestrator (normally Sol or Opus) may request a bounded Astra/Fable judgment task under the specialist and quota rules, or ask Derek to authorize a full orchestrator handoff. Prefer bounded assistance when it resolves the difficulty without transferring ownership. A difficulty-based handoff needs explicit authorization and a verified receiving session, context transfer, and ownership boundary; never create two active orchestrators. The opening rule still governs capability-loss handoffs. Do not replace the active orchestrator only because another model ranks higher.
...
Default shape: one ticket, one active worker at a time. The normal split inside a ticket is a sequential phase handoff, for example research to one worker and then decided implementation to another. Splitting a ticket requires the same one-line concrete reason that retaining work does. Running parallel helpers under one ticket additionally requires a stated separability claim — disjoint files, no shared schema or generated artifact — and a named integration owner, with one concurrent helper per ticket unless the orchestrator states why more is safe. Ticket outcome ownership never splits: it stays with the original worker under [bounded-assistance.md](references/bounded-assistance.md).
...
For substantial UI execution, prefer a bounded worker on a verified dispatch/return route, with the worker verifying its own adapter and required target state before any UI action; computer use is not inherently an orchestrator duty. Keep tiny UI actions local, or retain execution when only the orchestrator has an eligible adapter or delegation overhead outweighs the task, stating the concrete reason. Parent tool access does not prove child access. Give shared desktop input or the same mutable tab/window one active controller, including the orchestrator; worktree isolation alone does not isolate UI state. Name that controller in the dispatch and verify release or fence it before another session takes control, even for a tiny action; reinspect current state and preserve replay constraints at handback. `computer-use-routing` owns adapter eligibility, not the decision to keep all UI work with the orchestrator.
